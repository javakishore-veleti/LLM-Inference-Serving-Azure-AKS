...
**Qwen** is an LLM from **Alibaba**. Companies can download it and run it on their own GPUs instead of calling OpenAI, Claude, Copilot, or Gemini.
...
**Qwen** on that cluster is the Alibaba model named above. The model files go on **your GPUs** (Azure AKS or Runpod). In the picture, the chat sends a **prompt** to **vLLM**, vLLM sends it to **Qwen** (KV cache stays on the GPU), and the **response** comes back to the chat window.
